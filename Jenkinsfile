pipeline {
	...
	environment {
	    DOCKER_IMAGE_NAME = 'rajuchigicherla/forecast-server:latest'
	    DOCKER_USER_NAME = 'rajuchigicherla'
	}
	stages {
		stage('SCM Checkout') {
			steps {
				git branch: 'dev', credentialsId: '5d8c9e98-ddca-4df5-97cb-f333e6583cb0', url: 'https://github.com/Raju-Chigicherla/forecast-eureka-server.git'
			}
		}
		stage('mvn package') {
			steps {
